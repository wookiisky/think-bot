 ...
 */
syncManager.uploadData = async function() {
 ...
 */
syncManager.downloadData = async function() {
...
syncManager.fullSync = async function() {
  try {
    ...
    // Check if sync is configured before attempting full sync
    const isConfigured = await this.isConfigured();
    if (!isConfigured) {
    ...
    // Upload with merge (this now includes download, merge, and upload)
    const uploadResult = await this.uploadData();
    if (!uploadResult.success) {
      return uploadResult;
    }

    // Download the final merged data to ensure local consistency
 ...
 */
syncManager.isConfigured = async function() {
  try {
    const config = await syncConfig.getSyncConfig();
